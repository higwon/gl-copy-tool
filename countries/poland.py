...
def build_records(sheet, progress_callback=None):
    required = ["Konto", "Data księgowania", "Kwota Wn", "Kwota Ma", "Nazwa podmiotu", "Opis"]
    header_row, columns = find_header_row_and_columns(sheet, required)
    records = []
    total_rows = max(sheet.max_row - header_row, 1)

    for row_number in range(header_row + 1, sheet.max_row + 1):
        date = sheet.cell(row_number, columns["Data księgowania"]).value
        if not hasattr(date, "year"):
            continue
        account = sheet.cell(row_number, columns["Konto"]).value
        debit = sheet.cell(row_number, columns["Kwota Wn"]).value
        credit = sheet.cell(row_number, columns["Kwota Ma"]).value
        ...
        records.append(
            record(
                ...
                sheet.cell(row_number, columns["Nazwa podmiotu"]).value,
                sheet.cell(row_number, columns["Opis"]).value,
            )
        )
        if len(records) % 1000 == 0:
            percent = 35 + int(((row_number - header_row) / total_rows) * 7)
            report_progress(progress_callback, min(percent, 42), f"Poland GL 거래행을 읽는 중... ({len(records)}행)")
