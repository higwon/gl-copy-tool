...
1. Release에서 `GLInputCopyTool-v0.4.1.zip`을 다운로드합니다.
2. zip 압축을 풉니다.
3. 압축을 푼 폴더 안의 `GLInputCopyTool-v0.4.1.exe`를 실행합니다.
...
```powershell
.\dist\GLInputCopyTool\GLInputCopyTool-v0.4.1.exe
```
...
- 한국, 네덜란드, 오스트리아, 헝가리, 폴란드, 체코, 슬로바키아, 슬로베니아, 독일 국가 양식을 지원합니다.
...
- GL Auto 템플릿의 `차변(...)`, `대변(...)` 헤더는 괄호 안 통화 단위가 달라도 같은 컬럼으로 인식합니다.
...
- `gl_input_copy_gui.py`: 변환 실행 흐름과 tkinter UI
- `countries/registry.py`: 국가 선택값과 파서 등록
- `countries/common.py`: 헤더 탐색, 값 정규화, 복식부기 공통 변환
- `countries/<country>.py`: 국가별 고정 포맷 매핑
- `tests/test_country_parsers.py`: 국가별 최소 포맷 회귀 테스트

새 국가는 `countries` 폴더에 파서를 추가하고 registry에 등록하는 방식으로 확장합니다.
